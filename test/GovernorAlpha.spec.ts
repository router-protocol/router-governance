...
describe('GovernorAlpha', () => {
  ...
  it('router', async () => {
    const balance = await router.balanceOf(wallet.address)
    const totalSupply = await router.totalSupply()
  ...
  it('governor', async () => {
    ...
    const routerFromGovernor = await governorAlpha.router()
    expect(routerFromGovernor).to.be.eq(router.address)
  })
})
